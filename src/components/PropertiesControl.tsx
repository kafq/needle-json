...
const PropertiesControl:React.FC<Props> = (props) => {
    ...
    useEffect(() => {
        if (props.obj) {
          setSelectedKeys(getCollectionKeys(props.obj).reduce((acc, curr) => ((acc[curr] = false), acc), {}))
        }
    ...
    const renderButtons = () => {
        return Object.keys(props.obj[0]).map(i => {
          ...
          return <Button type={selectedKeys[i] ? 'SELECTED' : 'DEFAULT'} onClick={handleToggle} text={i}/>
        }) 
      }
